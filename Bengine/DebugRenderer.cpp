...
void Bengine::DebugRenderer::DrawSegment(const b2Vec2& a, const b2Vec2& b, const ColorRGBA8& color) {
	int i = m_verts.size();
	m_verts.resize(m_verts.size() + 2);
	m_verts[i].position = glm::vec2(a.x, a.y);
	m_verts[i].color = color;
	m_verts[i + 1].position = glm::vec2(b.x, b.y);
	m_verts[i + 1].color = color;
	m_indices.push_back(i);
	m_indices.push_back(i + 1);
}
void Bengine::DebugRenderer::DrawPoint(const b2Vec2& a, const ColorRGBA8& color)
{
	int i = m_verts.size();
...
void Bengine::DebugRenderer::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const Bengine::ColorRGBA8& color)
{
	int i = m_verts.size();
	m_verts.resize(m_verts.size() + 4);
	// Rotate the points
	m_verts[i].position = glm::vec2(vertices[0].x, vertices[0].y);
	m_verts[i + 1].position = glm::vec2(vertices[1].x, vertices[1].y);
	m_verts[i + 2].position = glm::vec2(vertices[2].x, vertices[2].y);
	m_verts[i + 3].position = glm::vec2(vertices[3].x, vertices[3].y);
	for (int j = i; j < i + 4; j++) {
...
void Bengine::DebugRenderer::drawCircle2D(const b2Vec2& center, float radius, const ColorRGBA8& color)
{
	int NUM_VERTS;
	int numVert = int(radius * 30.0f);
	if (numVert < 20)NUM_VERTS = 20;
	else if (numVert >= 20 && numVert < 100) NUM_VERTS = numVert;
	else if (numVert >= 100) NUM_VERTS = 100;
	// Set up vertices
